// 1
const randomArray = function createRandomArray(count, min, max) {
  const rand = () => Math.floor(Math.random() * (max - min) + min);
  const vals = Array.from({ length: count }).map(rand);
  return vals;
};
console.log(randomArray(10, 1, 13));
...
function filterEvenNumbers(arrayNumb) {
  ...
  const res = arrayNumb.filter(
    (element, index) => Number.isInteger(element) && element % 2 == 1
  );
  return res;
}
